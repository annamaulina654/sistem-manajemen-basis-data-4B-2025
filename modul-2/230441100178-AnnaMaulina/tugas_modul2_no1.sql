...
SELECT u.nama_usaha, p.nama_lengkap AS nama_pemilik, k.nama_kategori AS kategori_umkm, s.nama_skala AS skala_usaha,
kk.nama_kabupaten_kota, u.tahun_berdiri
FROM umkm u
...
SELECT p.nik, p.nama_lengkap, p.jenis_kelamin, p.nomor_telepon, p.email, u.nama_usaha
FROM pemilik_umkm p
JOIN umkm u ON p.id_pemilik = u.id_pemilik;

-- view produk umkm
CREATE VIEW view_produk_umkm AS
SELECT u.nama_usaha, p.nama_produk, p.deskripsi_produk, p.harga
FROM produk_umkm p
...
FROM umkm u
JOIN pemilik_umkm p ON u.id_pemilik = p.id_pemilik
JOIN skala_umkm s ON u.id_skala = s.id_skala WHERE nama_skala = 'Menengah' ;
